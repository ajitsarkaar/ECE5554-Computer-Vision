...
sizeOri = size(1:10:size(im{1}, 1) - 1, 2);
...
trackXFlow = track_x(:, 2) - track_x(:, 1);
trackYFlow = track_y(:, 1) - track_y(:, 1);

trackXFlow = reshape(trackXFlow, sizeOri, []);
trackYFlow = reshape(trackYFlow, sizeOri, []);

trackFlow(:,:,1) = trackXFlow;
trackFlow(:,:,2) = trackYFlow;

opticFlow = flowToColor(trackFlow);
